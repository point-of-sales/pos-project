...
class PhieuXuatController extends CPOSController
{
    ...
    {
        if (Yii::app()->user->checkAccess('Quanlynhapxuat.PhieuXuat.CapNhat')) {
            $model = $this->loadModel($id, 'PhieuXuat');
    ...
    {
        if (Yii::app()->user->checkAccess('Quanlynhapxuat.PhieuXuat.XoaGrid')) {
    ...
    {
        if (Yii::app()->user->checkAccess('Quanlynhapxuat.PhieuXuat.Xoa')) {
            if (Yii::app()->getRequest()->getIsPostRequest()) {
                $delModel = $this->loadModel($id, 'PhieuXuat');
                $message = '';
                $canDelete = true;
