...
class RWebUser extends CWebUser
{
    ...
    public function getCurrentUserBranchId()
    {
        if (isset(Yii::app()->user->id)) {
            $nhanVienHienHanh = NhanVien::model()->findByPk(Yii::app()->user->id);
            $chiNhanhHienHanh = ChiNhanh::model()->findByPk($nhanVienHienHanh->chi_nhanh_id);
            return $chiNhanhHienHanh->id;
        } else {
            return Yii::t('viLib', 'No branch');
        }
    }
